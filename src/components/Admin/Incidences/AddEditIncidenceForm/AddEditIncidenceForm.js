  ...
  return (
    <Form className='add-edit-incidence-form' onSubmit={formik.handleSubmit}>
     <div className='add-edit-incidence-form__all'>
        ...
        <br/>
        <Form.Input className='add-edit-incidence-form__date' label='Fecha' type='datetime-local' name='date' placeholder='Fecha'  value={formik.values.date} onChange={formik.handleChange} error={formik.errors.date} readOnly/>
...
function initialValues(data, data2){
  ...
  return {
      date_oc: data?.date || moment().format("YYYY-MM-DDThh:mm:ss"),
      ...
      date_rmv: data?.date_rmv || moment().format("YYYY-MM-DDThh:mm:ss"),
      third: data?.third || "",
...
function newSchema() {
  return {
    ...
    line_r: Yup.number().required(true),
    rmv: Yup.number().required(true),
